...
/**
 * Annotates sentences within HTML or plain text documents. The documents need
 * to be previously annotated with the Text Annotator. For each annotated text
 * span, this annotator will use the OpenNLP sentence detector to annotate
 * sentences.
 */
	...
	public void process(JCas jcas) throws AnalysisEngineProcessException {
		...
		for (Iterator<TextAnnotation> it = index.iterator(); it.hasNext();) {
			...
			if (UimaUtils.MIMETYPE_STRING.equals(jcas.getSofaMimeType())) {
				...
				String text = AnnotatorUtils.whiteoutHtml(inputAnnotation
						.getCoveredText());
